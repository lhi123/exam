<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="java.util.HashMap, java.util.ArrayList, vo.Dog" %>
...
<h2> 강아지 상품 목록 <a href="dogRegistForm.dog"> 강아지 상품 등록 </a></h2>
<table>
 <tr>
  <c:forEach var="dog" items="${dogList }" varStatus="status">
   <td>
    <a href="dogView.dog?id=${dog.id }">
     <img alt="" src="image/${dog.image }" id="productImage">
    </a>
    상품명 : ${dog.kind }<br>
    가격 : ${dog.price }<br>
   </td>
...
<c:if test="${todayImageList ne null }">
 <div id="todayImageList">
  <h2>오늘 본 강아지 상품 목록</h2>
  <table>
   <tr>
    <c:forEach var="todayImage" items="${todayImageList }">
     <td>
      <img alt="" src="image/${todayImage }" id="todayImage">
     </td>
      <c:if test="${((status.index+1) mod 4) == 0 }">
     </tr>
    <tr>
